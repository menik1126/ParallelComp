    ...
    def load_datas(self, data_file: str) -> None:
        ...
        with open(data_file) as fp:
            ...
            for line in fp:
                ...
                length_query = self.tokenizer(example["input"], return_tensors="pt")["input_ids"].shape[1]
                length_context = self.tokenizer(example["context"], return_tensors="pt")["input_ids"].shape[1]
                sum_len += length_context
                sum_len += length_query
...
#        assert 1==0
    ...
    def get_predicted(self,eval_batch_size=1,output_max_len=32):
        ...
        with accelerator.split_between_processes(self.datas) as split_data:
            results=dict(outputs=[], num_tokens=0,time_analysis={"time1":0,"tokens_sum":0,"time2":0} ,first_token_time=0)
            ...
            for i in tqdm(range(0, len(split_data), eval_batch_size)):
                ...
                if self.Truncation_Method=="WO_Truncation":
                    ...
                    # print("self.model.time1:",self.model.time1)
                    # print("self.model.tokens_generate:",self.model.tokens_generate)
                    # token_s =  self.model.tokens_generate /self.model.time1 
                    results["time_analysis"]["time1"] = self.model.time1  
                    results["time_analysis"]["time2"] = self.model.time2  
                    results["time_analysis"]["tokens_sum"] += self.model.tokens_generate
        ...
        if accelerator.is_main_process:
            time1=sum([r["time_analysis"]["time1"] for r in results_gathered ])
            time2 = sum([r["time_analysis"]["time2"] for r in results_gathered ])
            tokens_sum=sum([r["time_analysis"]["tokens_sum"] for r in results_gathered ])
            tokens_s = tokens_sum / time1
            print("tokens_s:",tokens_s)
            print("time1:",time1)
            print("tokens_sum:",tokens_sum)
            print("time2:",time2)
        # self.accelerator.wait_for_everyone()
    ...
    def run_experiment(self, batch_size: int = 1,output_max_len:int = 32):
        ...
        if accelerator.is_main_process:
            if self.data_name_full=="longbench":
                output_path = os.path.join("results/longbench_0330", f"{model_name}", datasets_name)
            elif self.data_name_full=="infinitebench":
                output_path = os.path.join("results/infinitebench_0330", f"{model_name}", datasets_name)
            else:
