...
def get_max_n_shots(train_df: pd.DataFrame, test_df: pd.DataFrame, tokenizer: PreTrainedTokenizerBase,
                    prompt_size: int) -> int:
    # print("====1")
    # print("TEXT_BETWEEN_SHOTS:{}".format(TEXT_BETWEEN_SHOTS))
    # print("====2")
    # 还要考虑TEXT_BETWEEN_SHOTS的影响
...
def filter_extremely_long_samples(df: pd.DataFrame, tokenizer: PreTrainedTokenizerBase, prompt_method=None) -> pd.DataFrame:
    """
    ...
    print("len of df0:{}".format(len(df)))
    if prompt_method =="other":
#       assert 1==0
