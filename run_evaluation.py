...
def get_dataset(dataset: str, tokenizer: PreTrainedTokenizerBase, prompt_method: str) -> (pd.DataFrame, pd.DataFrame, List):
...
def run_pcw_experiment(dataset: str, model: str, cache_dir: str, subsample_test_set: int, output_dir: str,
                       ...
                       random_seed: int, right_indentation: bool, prompt_method: str, output_json: str, model_class: str) -> None:
    print("n_windows:{}".format(n_windows))
    print("prompt_method:{}".format(prompt_method))
#    assert 1==0
    # 在这里传入模型的类
    pcw_model = load_pcw_wrapper(model, cache_dir, right_indentation, max(n_windows), prompt_method=prompt_method, model_class=model_class)

    test_df, train_df, labels = get_dataset(dataset, pcw_model.tokenizer, prompt_method)
...
#    assert 1==0
    ...
    em = ExperimentManager(test_df, train_df, pcw_model, labels, random_seed=random_seed,
                           n_shots_per_window=n_shots_per_window, subsample_test_set=subsample_test_set, prompt_method=prompt_method, output_json=output_json, n_windows=max(n_windows))
