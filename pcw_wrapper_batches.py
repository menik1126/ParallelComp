...
def combine_past_key_values_longbench_batches(model:PCWModelWrapper,
                                      past_lst: List[Tuple[Tuple[torch.Tensor]]], 
            ...
            if past_key_values: # generate
                ...
                if positional_sorting.lower() =="ntk":# 完全按照顺序即可
                    ...
                    if factor == 0:
                        ...
                        boundary_up[-1] = ralength_context_lenw_model_max_len
                    elif factor == 1:
                        n_task_tokens = position_ids.shape[1] - sum_windows_size
                        raw_model_max_len = position_dict["raw_model_max_len"]-n_task_tokens
                        n_windows = len(windows_rank)
...
class PCWModelWrapper:
    def __init__(self,
        ...
        self.time1,self.time2,self.time3=0,0,0
...
#           # 如果添加了query，希望context+query不超过一半
    ...
    def _get_windows_longbench(self, texts: List[str],context_max_len:int,
        ...
        if not self.dynamic_window:
            ...
            # assert 1==0
            encoded_input_window = {}
            encoded_input_window["input_ids"] = windows[0]['encoded_input']["input_ids"]
            encoded_input_window["attention_mask"] = windows[0]['encoded_input']["attention_mask"]
            
            output = self.model(**encoded_input_window)
            windows[0]['output'] = output
            # assert 1==0
        else:
            if self.topk_windows < 0:
            ...
            if "batches" in self.parallel_pattern:
                ...
                for raw_location,text in enumerate(texts):
                    ...
                    my_windows.append(window)
            # 开始padding
            print("len(my_windows):{}".format(len(my_windows)))
            ...
            print("output.shape:{}".format(output.logits.shape))
            print("output['past_key_values'][0][0].shape:{}".format(output['past_key_values'][0][0].shape))
            print("len(output)",len(output.logits.shape))
            # assert 1==0
            for i in range(len(my_windows)):
                new_output = {}
            ...
            # assert 1==0
            for raw_location,text in enumerate(texts):
                if "stream" in self.parallel_pattern or "batches" in self.parallel_pattern:
        ...
        # print("padding_len[select_index]",[padding_len[idx_item] for idx_item in select_index])
        else :
            padding_len = [padding_len[idx_item] for idx_item in select_index]
        return windows,windows_rank,select_index,output.past_key_values,padding_len
    ...
    def get_contexts_cache_longbench(self, contexts: List[str],context_max_len:int,
        ...
        windows,windows_rank,select_idx,batch_past_key_values,padding_len = \
        ...
        past_key_values = combine_past_key_values_longbench_batches(self,batch_past_key_values,
                                                                    query_len=[window['query_len'] for window in windows],
            ...
            if self.prompt_method=="complex_cot_pcw_multi_windows" \
                ...
                if "gemma" not in self.model_name:
                    ...
                    if "default" not in parallel_patterns:
                        self.tokens_generate  = self.tokens_generate + res[context_length:].shape[0]
                        res = self.tokenizer.decode(res[context_length:], skip_special_tokens=True)
                        
                    else:
                        self.tokens_generate  = self.tokens_generate + res.shape[0]
