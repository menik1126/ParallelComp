...
class LlamaForCausalLMPCW(LlamaForCausalLM, ABC):
    ...
    def prepare_inputs_for_generation(self,
        ...
        """
            tokenized_inputs_attention_mask shape:torch.Size([1, 86])                                                                                                                                                        | 0/1319 [00:00<?, ?it/s]
            combined_attention_mask shape:torch.Size([1, 3582])
            position_ids shape1:torch.Size([1, 86])
            attention_mask1:torch.Size([1, 3582])
            position_ids shape2:torch.Size([1, 3582])
            position_ids shape in llama:torch.Size([1, 3582])
            seq_length in llama:86
        """
        if attention_mask is not None and position_ids is None:
            # create PCW's position_ids on the fly  位置编码和question的位置编码对齐
...
class LlamaAttentionPCW(LlamaAttention):
 ...
 #       print("kv_seq_len0:{}".format(kv_seq_len))
